...
#pragma pack(pop)
  ...
  public:
    socket_t(boost::asio::io_context &ioc, std::function<void(tcp::socket &sock, launch_session_t &, msg_t &&)> &&handle_data_fn):
        handle_data_fn { std::move(handle_data_fn) }, sock { ioc } {}
  ...
  public:
    ...
    bind(net::af_e af, std::uint16_t port, boost::system::error_code &ec) {
      ...
      next_socket = std::make_shared<socket_t>(ioc, [this](tcp::socket &sock, launch_session_t &session, msg_t &&msg) {
