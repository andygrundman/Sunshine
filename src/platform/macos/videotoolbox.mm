...
namespace video {
  ...
  bool videotoolbox_encode_session_t::configure_session() {
    ...
    switch (config.videoFormat) {
      ...
      default:
        if (chroma444) {
          profile = kVTProfileLevel_H264_High444Predictive_AutoLevel;
        } else {
  ...
  bool videotoolbox_encode_session_t::init_encoder() {
    ...
    OSStatus err = VTCompressionSessionCreate(
      ...
      config.width,
      config.height,
      config.videoFormat == 1 ? kCMVideoCodecType_HEVC : kCMVideoCodecType_H264,
      encoder_spec,
      NULL,
      NULL,
      output_callback,
...
out:
  ...
  bool videotoolbox_encode_session_t::get_cached_pixel_buffer(CVPixelBufferRef *buf_out)
  {
    ...
    CVBufferSetAttachment(pixbuf, kCVImageBufferYCbCrMatrixKey, sunshine_to_vt_matrix(device->colorspace), kCVAttachmentMode_ShouldPropagate);
    CVBufferSetAttachment(pixbuf, kCVImageBufferColorPrimariesKey, sunshine_to_vt_primaries(device->colorspace), kCVAttachmentMode_ShouldPropagate);
    CVBufferSetAttachment(pixbuf, kCVImageBufferTransferFunctionKey, sunshine_to_vt_transfer(device->colorspace), kCVAttachmentMode_ShouldPropagate);
    const bool pq = device->colorspace.colorspace == colorspace_e::bt2020;
    if (pq) {
      const uint16_t hdr_nominal_peak_level = 1000;
      CFDataRef masteringDisplayColorVolume = sunshine_to_vt_masteringdisplay(hdr_nominal_peak_level);
      CFDataRef contentLightLevel = sunshine_to_vt_contentlightlevelinfo(hdr_nominal_peak_level);

      CVBufferSetAttachment(pixbuf, kCVImageBufferMasteringDisplayColorVolumeKey, masteringDisplayColorVolume, kCVAttachmentMode_ShouldPropagate);
      CVBufferSetAttachment(pixbuf, kCVImageBufferContentLightLevelInfoKey, contentLightLevel, kCVAttachmentMode_ShouldPropagate);
